...
* 建议使用行注释 (在 Sass 中是 `//`) 代替块注释，该注释不会被编译。
* 建议写企业站时使用scss，并使用kola编译器，增加样式可读性和缩小文件体积。
* 建议注释独占一行，避免行末注释。
* 给没有自注释的代码写上详细说明，比如：
  - 为什么用到了 z-index？需要和哪个元素做对比？
...
建议：在创建用于特定 JavaScript 的类名时，添加 `.js-` 前缀：
...
应避免使用 `@extend` 指令，因为它并不直观，而且具有潜在风险，特别是用在嵌套选择器的时候。即便是在顶层占位符选择器使用扩展，如果选择器的顺序最终会改变，也可能会导致问题。（比如，如果它们存在于其他文件，而加载顺序发生了变化）。其实，使用 @extend 所获得的大部分优化效果，gzip 压缩已经足够了，因此只需要通过 mixin 让样式表更符合 DRY 原则就足够了。
...
如果你始终坚持要使用 ID 选择器（劝你三思），那也不应该嵌套它们。如果打算这么做，需要先重新检查你的标签，或者指明原因。如果想要写出风格良好的 HTML 和 CSS，是**不**应该这样做的。
